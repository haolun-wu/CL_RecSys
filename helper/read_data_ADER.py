...
class Data(object):
    def __init__(self, data_dir, val_ratio=None, test_ratio=0.2, user_filter=5, item_filter=5, seed=0):
        ...
        # origin_data_path = os.path.join(data_dir, "DIGINETICA/period.dat")
        ...
        for i in range(3):
            data_path = os.path.join(data_dir, "period_{}.txt".format(i))
            df = digi(data_path).load().reset_index().drop(['index'], axis=1)
            df['rate'] = 1
            data_full.append(df)

        a = list(data_full[0]['user'])
        b = list(data_full[1]['user'])
        print(sorted(a)[:5])
        print(sorted(b)[:5])
        # c = list(data_full[2]['user'])
        # d = list(data_full[3]['user'])
        # e = list(data_full[4]['user'])
        #
        # a = list(df1['user'])
        # b = list(df2['user'])
        print("common:", len(list(set(a) & set(b))))
        # print("common:", len(list(set(b) & set(c))))
        # print("common:", len(list(set(c) & set(d))))
        # print("common:", len(list(set(d) & set(e))))

        # return

        # data_path = os.path.join(data_dir, "DIGINETICA/period_0.txt")
        # data_0 = digi(data_path).load().reset_index().drop(['index'], axis=1)
        # data_0['rate'] = 1
        # print("data_0:", data_0)
        #
        ...
        a = list(self.data_full_dict[0][0].keys())
        b = list(self.data_full_dict[1][0].keys())
        print("common:", len(list(set(a) & set(b))))
        # print("U-I:")
...
if __name__ == '__main__':
    ...
    data_dir = "C:/Users/eq22858/Documents/GitHub/CL_RecSys/data/DIGINETICA"
