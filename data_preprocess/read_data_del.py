...
class mv(DatasetLoader):
    def __init__(self, data_dir):
        self.fpath_rate = os.path.join(data_dir, 'user_taggedbookmarks.dat')
        self.fpath_tag = os.path.join(data_dir, 'bookmark_tags.dat')
    ...
    def load(self):
        ...
        df_rate = pd.read_csv(self.fpath_rate,
                              sep='\t',
                              engine='python',
                              names=['user', 'item', 'tag', 'day', 'month', 'year', 'hour', 'minute', 'second'],
                              usecols=['user', 'item'],
...
class Data(object):
    def __init__(self, data_dir, val_ratio=None, test_ratio=0.2, user_filter=5, item_filter=5, seed=0):
        ...
        df = df[df['tag'].apply(lambda x: len(set(x)) >= 5)]  # make sure each item has 5 tags
        # df = self.remove_infrequent_tags(df, 5)
        df = self.remove_infrequent_items(df, item_filter)
        df = self.remove_infrequent_users(df, user_filter)
        ...
        # draw_distribution(df[['user', 'item']], 'user', 'item', "del")
        # draw_distribution(df[['user', 'item']], 'item', 'user', "del")
        # draw_distribution(IT_df, 'item', 'tag', "del")
        # draw_distribution(IT_df, 'tag', 'item', "del")
...
def parser_args():
    ...
    parser.add_argument('--item_filter', type=int, default=3)
    return parser.parse_args()


if __name__ == '__main__':
    parser = parser_args()
    data_dir = '../data/hetrec-del'
