...
class Read_kgat(DatasetLoader):
    ...
    def load(self):
        ...
        _, u_dict_train = self.load_ratings(self.fpath + '/train.txt')
        _, u_dict_test = self.load_ratings(self.fpath + '/test.txt')
        # u_dict = u_dict_train.copy()
        # u_dict_train.update(u_dict_test)
...
class Data(object):
    def __init__(self, data_dir, data_name, val_ratio=None, test_ratio=0.2, user_filter=5, item_filter=5, seed=0):
        ...
        if os.path.exists(file_path):
            ...
            df = df[df['tag'].apply(lambda x: len(set(x)) >= 3)]
        ...
        item_tag_count = IT_df['item'].value_counts().to_frame()
